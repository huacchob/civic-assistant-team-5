...
class SupabaseClient:
    ...
    async def query_home_by_id(
        self, home_id
    ):  # Wrappers are for parameter validation, error handling, and domain interface
    ...
    async def search_programs_rag(self, embedding, limit=10):
        ...
        # Execute the query
        result = await self.session.call_tool("execute_sql", {"query": query_sql})
    ...
    def _parse_price_data(self, result):
        ...
        try:
            ...
            return {
                ...
                "average_sale_price": round(float(data.get("average_sale_price", 0)), 2)
                if data.get("average_sale_price")
                else 0,
                "min_sale_price": round(float(data.get("min_sale_price", 0)), 2)
                if data.get("min_sale_price")
                else 0,
                "max_sale_price": round(float(data.get("max_sale_price", 0)), 2)
