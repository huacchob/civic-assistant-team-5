...
def format_planner_results(result: Any) -> str:
    """Format the planner agent results for display."""
    lines: list[Any] = []

    if result.get("budgeting_agent_results"):
        budget: Any = result["budgeting_agent_results"]
        lines.append(
            "<strong>Profile:</strong> "
            + f"Income: {fmt_money(budget.get('income'))} | "
        ...
        budget_result: Any = (budget.get("budget_result") or {}).get("budget", {})
        loan_result: Any = (budget.get("loan_result") or {}).get("max_loan", {})

        financial_info: list[str] = []
        if budget_result.get("budget"):
            financial_info.append(
                f"Yearly Budget: {fmt_money(budget_result.get('budget'))}"
            )
        if loan_result.get("max_loan"):
            financial_info.append(f"Max Loan: {fmt_money(loan_result.get('max_loan'))}")
    ...
    analysis: str = result.get("final_analysis", "No analysis available")
    if analysis and analysis != "No analysis available":
        lines.append("<strong>Analysis:</strong>")
        lines.append(analysis)
    else:
        lines.append(
            "<strong>Status:</strong> Analysis unavailable - please try again."
        )
...
async def run_planner_with_ui(
    ...
    zip_code: str,
    building_class: str,
    residential_units: str,
    gross_square_feet: str,
    year_built: str,
    current_debt: str,
) -> str:
    """Parse and validate user input, then call the planner agent."""
    try:
        if not income:
        ...
        return format_planner_results(result=result)
...
def create_interface() -> gr.Blocks:
    """Create the Gradio Blocks UI interface."""
    with gr.Blocks(
        title="MAREA",
    ...
    """,
        ...
        gr.Markdown(
            value="# <span style='display: inline-flex; align-items: center;'><svg width='20' height='20' viewBox='0 0 24 24' fill='none' xmlns='http://www.w3.org/2000/svg' style='margin-right: 8px;'><path d='M12 0C5.374 0 0 5.373 0 12 0 17.302 3.438 21.8 8.207 23.387c.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23A11.509 11.509 0 0112 5.803c1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576C20.566 21.797 24 17.3 24 12c0-6.627-5.373-12-12-12z' fill='#6B7280'/></svg>MAREA</span>"
        )
        gr.Markdown(
            value="<hr style='border: none; border-top: 1px solid #6B7280; margin: 3px 0 2px 0; width: 40%;'>"
        )
        gr.Markdown(
            value="<span style='color: #6B7280;'>Smarter Home Buying, Powered by AI.</span>"
        )
        ...
        with gr.Row():
            with gr.Column(elem_id="left-panel"):
                ...
                income = gr.Number(
                    label="Net Annual Income ($)", value=75000, minimum=0, step=1000
                )
                current_debt = gr.Number(
                    label="Current Debt ($)", value=0, minimum=0, step=1000
                )
                credit_score = gr.Number(
                    label="Credit Score", value=720, minimum=300, maximum=850, step=10
                )

                # Property Information
                target_home_id = gr.Number(
                    label="Target Home ID", value=7, minimum=1, step=1
                )
                ...
                gross_square_feet = gr.Number(
                    label="Gross Square Feet", value=1200, minimum=1, step=100
                )
                year_built = gr.Number(
                    label="Year Built", value=2000, minimum=1800, maximum=2024, step=1
                )

                analyze_btn = gr.Button(value="Analyze", variant="primary", size="lg")
